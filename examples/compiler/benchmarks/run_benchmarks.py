...
def compile_eigenscript(source_file):
    ...
    result = subprocess.run(
        ['python3', compiler, source_file, '--exec'],
        capture_output=True,
        text=True
    )
...
def main():
    ...
    if results:
        ...
        for r in results:
            print(f"  {r['desc']}")
            if r['speedup'] >= 1.0:
                print(f"    {r['speedup']:.1f}x speedup (LLVM: {r['llvm']*1000:.1f}ms, Python: {r['python']*1000:.1f}ms)")
            else:
                slowdown = 1.0 / r['speedup']
                print(f"    {slowdown:.1f}x slower (LLVM: {r['llvm']*1000:.1f}ms, Python: {r['python']*1000:.1f}ms)")
        
        fast_results = [r for r in results if r['speedup'] >= 1.0]
        if fast_results:
            avg_speedup = sum(r['speedup'] for r in fast_results) / len(fast_results)
            print()
            print(f"  Average speedup (excluding slower cases): {avg_speedup:.1f}x faster")
        print()
