...
from eigenscript.parser.ast_builder import (
    ...
    Assignment, FunctionDef, Return, Break,
    Conditional, Loop, Relation, Interrogative,
    ListLiteral, Index, Slice, Program
)
...
class LLVMCodeGenerator:
    ...
    def _declare_runtime_functions(self):
        ...
        eigen_create_type = ir.FunctionType(
            self.eigen_value_ptr,
            [self.double_type]
        )
        self.eigen_create = ir.Function(self.module, eigen_create_type, name="eigen_create")
        
        # eigen_update(eigen*, new_value) -> void
        eigen_update_type = ir.FunctionType(
            self.void_type,
            [self.eigen_value_ptr, self.double_type]
        )
        self.eigen_update = ir.Function(self.module, eigen_update_type, name="eigen_update")
        
        # eigen_get_value(eigen*) -> double
        eigen_get_value_type = ir.FunctionType(
            self.double_type,
            [self.eigen_value_ptr]
        )
        self.eigen_get_value = ir.Function(self.module, eigen_get_value_type, name="eigen_get_value")
        ...
        self.eigen_get_gradient = ir.Function(self.module, eigen_get_gradient_type, name="eigen_get_gradient")
        ...
        self.eigen_get_stability = ir.Function(self.module, eigen_get_stability_type, name="eigen_get_stability")
        ...
        self.eigen_get_iteration = ir.Function(self.module, eigen_get_iteration_type, name="eigen_get_iteration")
        ...
        self.eigen_check_converged = ir.Function(self.module, eigen_check_converged_type, name="eigen_check_converged")
        ...
        self.eigen_check_diverging = ir.Function(self.module, eigen_check_diverging_type, name="eigen_check_diverging")
        ...
        self.eigen_check_oscillating = ir.Function(self.module, eigen_check_oscillating_type, name="eigen_check_oscillating")
        ...
        self.eigen_check_stable = ir.Function(self.module, eigen_check_stable_type, name="eigen_check_stable")
        ...
        self.eigen_check_improving = ir.Function(self.module, eigen_check_improving_type, name="eigen_check_improving")
        
        # List runtime functions
        # eigen_list_create(length) -> EigenList*
        eigen_list_create_type = ir.FunctionType(
            self.eigen_list_ptr,
            [self.int64_type]
        )
        self.eigen_list_create = ir.Function(self.module, eigen_list_create_type, name="eigen_list_create")
        
        # eigen_list_get(list*, index) -> double
        eigen_list_get_type = ir.FunctionType(
            self.double_type,
            [self.eigen_list_ptr, self.int64_type]
        )
        self.eigen_list_get = ir.Function(self.module, eigen_list_get_type, name="eigen_list_get")
        
        # eigen_list_set(list*, index, value) -> void
        eigen_list_set_type = ir.FunctionType(
            self.void_type,
            [self.eigen_list_ptr, self.int64_type, self.double_type]
        )
        self.eigen_list_set = ir.Function(self.module, eigen_list_set_type, name="eigen_list_set")
        
        # eigen_list_length(list*) -> i64
        eigen_list_length_type = ir.FunctionType(
            self.int64_type,
            [self.eigen_list_ptr]
        )
        self.eigen_list_length = ir.Function(self.module, eigen_list_length_type, name="eigen_list_length")
    ...
    def _generate_literal(self, node: Literal) -> ir.Value:
        ...
        if node.literal_type == "number":
            ...
            string_const = ir.Constant(ir.ArrayType(self.int8_type, len(string_val)),
                                      bytearray(string_val.encode('utf-8')))
            global_str = ir.GlobalVariable(self.module, string_const.type, name=f"str_{id(node)}")
            global_str.global_constant = True
            global_str.initializer = string_const
            return self.builder.bitcast(global_str, self.string_type)
        else:
            raise NotImplementedError(f"Literal type {node.literal_type} not implemented")
    
    def _generate_identifier(self, node: Identifier) -> ir.Value:
        """Generate code for variable access."""
        # Check if this is a predicate
        if node.name in ['converged', 'diverging', 'oscillating', 'stable', 'improving']:
        ...
        if isinstance(loaded_ptr.type, ir.PointerType):
            ...
            if (isinstance(pointed_type, ir.LiteralStructType) and 
                len(pointed_type.elements) == 3 and
                isinstance(pointed_type.elements[0], ir.PointerType)):
    ...
    def _generate_unary_op(self, node: UnaryOp) -> ir.Value:
        ...
        if isinstance(node.operand, Identifier):
            ...
            if predicate_name in ['converged', 'diverging', 'oscillating', 'stable', 'improving']:
                # Get the last assigned variable (simplified - should be explicit in real code)
                if self.local_vars:
                    ...
                    if predicate_name == 'converged':
                        result = self.builder.call(self.eigen_check_converged, [eigen_ptr])
                    elif predicate_name == 'diverging':
                        result = self.builder.call(self.eigen_check_diverging, [eigen_ptr])
                    elif predicate_name == 'oscillating':
                        result = self.builder.call(self.eigen_check_oscillating, [eigen_ptr])
                    elif predicate_name == 'stable':
                        result = self.builder.call(self.eigen_check_stable, [eigen_ptr])
                    elif predicate_name == 'improving':
                        result = self.builder.call(self.eigen_check_improving, [eigen_ptr])
                    else:
                        raise NotImplementedError(f"Predicate {predicate_name} not implemented")
    ...
    def _generate_relation(self, node: Relation) -> ir.Value:
        ...
        if isinstance(node.left, Identifier):
            ...
            if func_name == "print":
                ...
                # Print format string
                fmt_str = "%f\n\0"
                fmt_const = ir.Constant(ir.ArrayType(self.int8_type, len(fmt_str)),
                                       bytearray(fmt_str.encode('utf-8')))
                global_fmt = ir.GlobalVariable(self.module, fmt_const.type, name=f"fmt_{id(node)}")
            ...
            if func_name in self.functions:
                ...
                if isinstance(arg, Identifier):
                    if arg.name in self.local_vars or arg.name in self.global_vars:
                        var_ptr = self.local_vars.get(arg.name) or self.global_vars.get(arg.name)
                        eigen_ptr = self.builder.load(var_ptr)
                    else:
        ...
        raise NotImplementedError(f"Relation {node.left} of {node.right} not implemented")
    ...
    def _generate_conditional(self, node: Conditional) -> None:
        ...
        else_block = self.current_function.append_basic_block(name="if.else") if node.else_block else None
    ...
    def _generate_interrogative(self, node: Interrogative) -> GeneratedValue:
        ...
        if not isinstance(target_expr, Identifier):
            raise NotImplementedError("Interrogatives only support simple identifiers for now")
        ...
        # Other interrogatives extract specific fields and return scalars
            ...
            raise NotImplementedError(f"Interrogative {node.interrogative} not implemented")
    ...
    def _generate_function_def(self, node: FunctionDef) -> None:
        ...
        func_type = ir.FunctionType(
            self.double_type,
            [self.eigen_value_ptr]  # Single parameter passed via "of"
        )
