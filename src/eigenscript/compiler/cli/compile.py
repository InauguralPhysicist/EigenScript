...
def compile_file(input_file: str, output_file: str = None, emit_llvm: bool = True, verify: bool = True, link_exec: bool = False, opt_level: int = 0):
    ...
    try:
        ...
        if link_exec:
            ...
            if not os.path.exists(runtime_o) or os.path.getmtime(runtime_c) > os.path.getmtime(runtime_o):
...
Examples:
  ...
  %(prog)s program.eigs --no-verify  # Skip verification
    ...
    parser.add_argument('-o', '--output', help='Output file (default: input with .ll or .o extension)')
    parser.add_argument('--obj', action='store_true', help='Compile to object file instead of LLVM IR')
    parser.add_argument('--exec', action='store_true', help='Compile and link to executable')
    parser.add_argument('--no-verify', action='store_true', help='Skip LLVM module verification')
